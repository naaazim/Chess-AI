...
public class VueEchiquier extends GridPane {
    ...
    private static final Color COULEUR_CLAIR = Color.rgb(240, 217, 181);
    private static final Color COULEUR_FONCE = Color.rgb(181, 136, 99);
    private static final Color COULEUR_SELECTION = Color.rgb(100, 255, 100, 0.5);
    private static final Color COULEUR_DERNIER_COUP = Color.rgb(255, 255, 100, 0.5);
        ...
        for (int lig = 0; lig < 8; lig++) {
            for (int col = 0; col < 8; col++) {
                ...
                if (selection != null && selection.equals(caseCourante)) {
                    highlight.setFill(COULEUR_SELECTION); // Vert pour la sélection
                } else if (dernierCoup != null
                        && (dernierCoup.depart().equals(caseCourante) || dernierCoup.arrivee().equals(caseCourante))) {
                    highlight.setFill(COULEUR_DERNIER_COUP); // Jaune pour dernier coup
                }

                // Pièce (Node)
                Node pieceNode = ressources.getRepresentation(p, TAILLE_CASE);
                if (pieceNode != null) {
                    pieceNode.setMouseTransparent(true); // Laisser le clic passer au stack
                }
                ...
                // Coordonnées sur les bords (optionnel, pour debug/confort)
                if (col == 0) {
